...
export default function AdminUserStatsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const [stats, setStats] = useState<Stats | null>(null);
  const [loading, setLoading] = useState(true);
  const [period, setPeriod] = useState("month");
  const [userName, setUserName] = useState("Пользователь"); // Could fetch name separately if needed
  ...
  const fetchStats = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/users/${id}/stats?${params}`);
      if (res.ok) {
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link
            href="/admin/users"
            className="p-2 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 transition"
          >
            <ArrowLeft size={20} />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Статистика пользователя</h1>
            <p className="text-slate-500 dark:text-slate-400">Детальный обзор показателей</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-sm font-medium text-slate-500 dark:text-slate-400">Прибыль польз.</span>
              </div>
