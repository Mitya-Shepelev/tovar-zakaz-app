...
export default function TicketChatPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] animate-in fade-in duration-500">
      {/* Header */}
      <div className="flex items-center gap-4 mb-6 bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <Link
        ...
        <div>
          <h1 className="text-lg font-bold text-slate-900 dark:text-white">Чат с поддержкой</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">ID: {id}</p>
        </div>
      </div>

      {/* Messages Area */}
      <div className="flex-1 overflow-y-auto bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 space-y-6 mb-4 relative">
        {messages.map((msg) => {
      ...
      <div className="relative">
        ...
        {attachments.length > 0 && (
          <div className="absolute bottom-full left-0 mb-2 flex gap-2 p-2 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-lg z-10">
        ...
        <form onSubmit={sendMessage} className="relative flex items-center gap-2">
          <div className="relative flex-1">
            <input
              ...
              className="w-full pl-12 pr-12 py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-sm transition-all"
            />
            ...
            <button
              ...
              disabled={uploading}
              className="absolute right-3 top-1/2 -translate-y-1/2 p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors disabled:opacity-50"
            >
              {uploading ? (
